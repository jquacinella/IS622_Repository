...
(d) The number of key value pairs generated from M will be the number of columns of N, $c_N$, times the number of rows of M, $r_M$. Similar logic for matrix N, we have $c_Mr_N$. The reducer must produce $r_Mc_N$ tuples as the final output. The order of the algorithm however, is $O(r_Mc_N + c_Mr_N + r_M*c_N)$. Since $c_M = r_N$ for matrix multiplication to work, this reduces to $O(2r_Mc_N + {c_M}^2)$.
...
(c) The grouping and aggregation on the relation R(A, B), where A
is the grouping attribute and B is aggregated by the MAX operation.
Assume A and B have domains of size a and b, respectively.
